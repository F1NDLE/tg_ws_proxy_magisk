...
if [ ! -f "$MODPATH/python/lib/libandroid-posix-semaphore.so" ]; then
    LSPD="/data/data/com.termux/files/usr/lib/libandroid-posix-semaphore.so"
    [ -f "$LSPD" ] && cp "$LSPD" "$MODPATH/python/lib/" && chmod 755 "$MODPATH/python/lib/libandroid-posix-semaphore.so"
fi
